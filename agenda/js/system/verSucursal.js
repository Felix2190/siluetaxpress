...
function iniciar(){
	...
	setTimeout(function() {
		$("#slcEstado").val($("#hdnEstado").val());
		cambioMunicipio();
	},800);
	setTimeout(function() {
		$("#slcMunicipio").val($("#hdnMunicipio").val());
		$("#slcHrEntreI").val($("#hdnHrEntreE").val());
		$("#slcHrEntreF").val($("#hdnHrEntreS").val());
		$("#slcHrSabI").val($("#hdnHrSabadoE").val());
		$("#slcHrSabF").val($("#hdnHrSabadoS").val());
	},1300);
...
function altaSucursal(){
	...
	var numCon = parseInt($("#txtConsultorios").val());
	var numCa = parseInt($("#txtCabinas").val());
	
	var hdnNumCon =parseInt( $("#hdnConsultorios").val());
	var hdnNumCa = parseInt($("#hdnCabinas").val());
	...
	if(existeError){
		...
		if(hdnNumCon>numCon){
			mostrarMsjError('El n&uacute;mero de consultorios no debe ser menor al ya registrado', 3);
			return false;
		}
		if(hdnNumCa>numCa){
			mostrarMsjError('El n&uacute;mero de cabinas no debe ser menor al ya registrado', 3);
			return false;
		}
		numCon-=hdnNumCon;
		numCa-=hdnNumCa;
		mostrarMsjEspera('Espere un momento... guardando informaci&oacute;n.', 3);
		xajax_guardarSucursal(ent,mun,sucursal,direccion,hrEntreI,hrEntreF,hrSabI,hrSabF,numCon,numCa,idSucursal);
	}
}
