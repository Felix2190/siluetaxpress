...
if ($dia != 7) { // no es domingo
    ...
    $query = "Select MIN(entreSemanaEntrada) as entre, MIN(sabadoEntrada) as sabado FROM sucursal";
    ...
    //echo $query;
     if ($resultado && mysqli_num_rows($resultado) > 0) {
        while ($row_inf = mysqli_fetch_assoc($resultado)){
             $sms=enviaSMS_recordatorio("52".$row_inf['telefonoCel'], $row_inf['nombre'], $row_inf['tipoConsulta'], date("d/m/Y",strtotime($row_inf['fecha'])), $row_inf['hora'], $row_inf['sucursal'], $row_inf['idCita']);
             if ($sms)
